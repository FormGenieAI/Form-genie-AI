...
# -----------------------------------------------------------------------------
# Inspector v2 - standalone account baseline
# -----------------------------------------------------------------------------
...
resource "aws_sns_topic_subscription" "email" {
  for_each  = var.enable_notifications ? toset(var.notification_emails) : toset([])
...
resource "aws_cloudwatch_event_rule" "guardduty_findings" {
  count       = var.enable_notifications && var.enable_guardduty ? 1 : 0
...
resource "aws_cloudwatch_event_rule" "securityhub_findings" {
  count       = var.enable_notifications && var.enable_securityhub ? 1 : 0
...
resource "aws_config_delivery_channel" "this" {
  ...
  depends_on = [
    ...
    aws_config_configuration_recorder.this # 👈 ADD THIS
  ]
}
